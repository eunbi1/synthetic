...
def animation_gif(trajectory,name='test.gif'):
    fig, ax = plt.subplots()

    i=0
    ims = []
    plt.scatter(trajectory[-1, 0, 0], trajectory[-1, 0, 1], s=1, c='red', alpha=0.5)
    plt.scatter(trajectory[-1, -1, 0], trajectory[-1, -1, 1], s=1, c='blue', alpha=0.5)
    cmap = get_cmap(len(trajectory))
    for j in range(np.shape(trajectory)[1]):
     for i in range(len(trajectory)):

      line = Line2D(trajectory[i, :j+1, 0], trajectory[i, :j+1, 1],alpha=0.2, c=cmap(i))
     ims.append(plt.plot(*line.get_data(),alpha=0.2, c=cmap(i)))
...
def runner(alpha,  dataset= "swiss roll",train_max_iter=100,batch_size=1000,
    ...
    if dimension >1:
        ...
        plt.scatter(X, Y, s=1, alpha=0.03, label = f' ratio{ratio},Alpha{alpha} predict:{average_ratio:.1f}FID:{average_fid:.4f}')
    ...
    plt.savefig(name)
    name = sampler+ str(ratio)+'frac'+str(dimension)+ label_mode+''+noise_mode+dataset+str(alpha)+'.gif'
    name = os.path.join(dir_path, name)
    animation_gif(trajectory,name)
...
#,(1.8, 'fDSM', 'independent'), (1.8, 'fDSM', 'isotropic'),
#(1.5, 'fDSM', 'isotropic')
 for alpha, label_mode, noise_mode in [(2.0, 'fDSM', 'isotropic'),(1.5, 'fDSM', 'isotropic'),(1.2, 'fDSM', 'isotropic')]:
  for dataset in ["mixture of gaussian"]:
    F = []
    for ratio in fraction:
     if dataset== 'mog':
...
# sample(1.8, path="/scratch/private/eunbiyoon/LIM-synthetic-data/10_fDSM__isotropic1.8ckpt.pth",noise_mode="isotropic", label_mode = "fDSM")def animation_gif(trajectory):
    fig, ax = plt.subplots()
    i=0
